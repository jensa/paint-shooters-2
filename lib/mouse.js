...
THREEx.MouseState = function()
{
        ...
        var self = this;
        this._onMouseDown = function(event){ self._onMouseButtonChange(event, true); };
        this._onMouseUp = function(event){ self._onMouseButtonChange(event, false); };
        this._onMouseMove = function(event){ self._onMouseMovePosition(event); };
...
THREEx.MouseState.prototype._onMouseMovePosition = function(event)
{
  this.mouseX = event.clientX;
  this.mouseY = event.clientY;
}

THREEx.MouseState.prototype._onMouseButtonChange = function(event, pressed)
{
        // log to debug
...
//query mouse state: buttonDesc is a string describing which buttons you want to query
// lmb, rmb, mmb
...
THREEx.MouseState.prototype.moved = function(){
  var newPos = this.position();
  var sameX = this.lastPosition.x + 1 > newPos.x && this.lastPosition.x - 1 < newPos.x;
  var sameY = this.lastPosition.y + 1 > newPos.y && this.lastPosition.y - 1 < newPos.y;
  if(sameX && sameY)
    return false;
  return true;
}
...
THREEx.MouseState.prototype.position = function(){
  return {x:round(this.mouseX), y:round(this.mouseY)};
}
